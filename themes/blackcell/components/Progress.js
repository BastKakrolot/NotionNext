...
const Progress = ({ targetRef, showPercent = true }) => {
  ...
  const scrollListener = () => {
    ...
    if (target) {
      ...
      const clientBottom = rect.bottom;
      const scrollY = clientHeight - clientBottom + screenHeight;
  ...
  return (
    <div className="h-2 w-full shadow-2xl bg-hexo-light-gray dark:bg-black">
      <div
        className="h-2 bg-gray-600 duration-200"
        style={{ width: `${percent}%` }}
      >
        {showPercent && (
